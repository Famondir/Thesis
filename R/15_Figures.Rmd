...
```{r real-table-extraction-qwen235-by-company-unput-type-plot, echo=echo_flag, warning=warning_flag, message=message_flag, out.width="100%", out.height="95%", out.extra='keepaspectratio', fig.height=9, fig.width=16, dev=std_dev, fig.cap="Percentage of correct predictions total with Qwen-235B grouped by company and input type.", cache=cache_large_figures}
...
```{r compare-real-table-extraction-by-context-type, echo=echo_flag, warning=warning_flag, message=message_flag, out.width="100%", out.height="95%", fig.height=10, dev=std_dev, out.extra='keepaspectratio', fig.cap="Comparing percentage of correct extracted values overall for real Aktiva extraction task with synthetic and real examples for in-context learning"}
bind_rows(
...
```{r compare-real-table-extraction-by-context-type-numeric, echo=echo_flag, warning=warning_flag, message=message_flag, out.width="100%", out.height="95%", fig.height=10, dev=std_dev, out.extra='keepaspectratio', fig.cap="Comparing percentage of correct extracted numeric values for real Aktiva extraction task with synthetic and real examples for in-context learning"}
bind_rows(
  df_real_table_extraction_synth %>% mutate(context = "synth"), 
  df_real_table_extraction %>% mutate(context = "real")
  ) %>% 
  ...
  ggplot() +
  geom_boxplot(aes(fill=context, y = percentage_correct_numeric), alpha = .5) +
  # scale_fill_manual(values = c("blue", "orange")) +
  scale_x_discrete(guide = guide_axis(angle = 30)) +
  facet_nested(method_family+n_examples~model)
```

```{r compare-real-table-extraction-by-context-type-F1, echo=echo_flag, warning=warning_flag, message=message_flag, out.width="100%", out.height="95%", fig.height=10, dev=std_dev, out.extra='keepaspectratio', fig.cap="Comparing the F1 score for real Aktiva extraction task with synthetic and real examples for in-context learning"}
bind_rows(
  df_real_table_extraction_synth %>% mutate(context = "synth"), 
  df_real_table_extraction %>% mutate(context = "real")
  ) %>% 
  ...
  ggplot() +
  geom_boxplot(aes(fill=context, y = NA_F1), alpha = .5) +
  # scale_fill_manual(values = c("blue", "orange")) +
  scale_x_discrete(guide = guide_axis(angle = 30)) +
  facet_nested(method_family+n_examples~model)
```

```{r compare-effect-of-respecting-units-on-overall-performance, echo=echo_flag, warning=warning_flag, message=message_flag, out.width="100%", out.height="95%", fig.height=12, out.extra='keepaspectratio', dev=std_dev, fig.cap="Comparing the effect on overall perfrormance if currency units should be respected on all predictions and specifically on predictions where all or just some columns have units.", cache=cache_large_figures}
...
```{r compare-effect-of-respecting-units-on-numeric-performance, echo=echo_flag, warning=warning_flag, message=message_flag, out.width="100%", out.height="95%", out.extra='keepaspectratio', fig.height=12, dev=std_dev, fig.cap="Comparing the effect on numeric perfrormance if currency units should be respected on all predictions and specifically on predictions where all or just some columns have units.", cache=cache_large_figures}
df_real_table_extraction_synth_respect_units %>% 
  ggplot() +
...
```{r compare-effect-of-respecting-units-on-NA-F1, echo=echo_flag, warning=warning_flag, message=message_flag, out.width="100%", fig.height=12, out.height="95%", dev=std_dev, out.extra='keepaspectratio', fig.cap="Comparing the effect on NA F1 score if currency units should be respected on all predictions and specifically on predictions where all or just some columns have units.", cache=cache_large_figures}
df_real_table_extraction_synth_respect_units %>% 
  ggplot() +
