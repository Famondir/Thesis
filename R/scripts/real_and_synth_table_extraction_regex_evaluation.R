...
df %>%
  select(c(model, percentage_correct_numeric, percentage_correct_total, T_EUR, method, sum_same_line)) %>% 
  pivot_longer(cols = -c(model, T_EUR, method, sum_same_line)) %>% 
  ggplot() +
  geom_boxplot(aes(x = model, y = value, fill = method), alpha = .3) +
  geom_point(
    data= . %>% filter(model == "real_tables", name == "percentage_correct_numeric"),
    aes(x = model, y = value, alpha = method), color = "#264DEB",
    shape = 4, position=position_jitterdodge(dodge.width=0.9, jitter.width = 0.2, jitter.height = 0.005)
  ) +
  # geom_point(
  #   data= . %>% filter(model == "synth_tables", name == "percentage_correct_numeric", sum_same_line),
  #   aes(x = model, y = value, alpha = method), color = "#264DEB",
  #   shape = 4, position=position_jitterdodge(dodge.width=0.9, jitter.width = 0.2, jitter.height = 0.005)
  # ) +  
...
df %>% select(c(model, NA_precision, NA_recall, NA_F1, T_EUR, method, sum_same_line)) %>% 
  pivot_longer(cols = -c(model, T_EUR, method, sum_same_line)) %>% 
  ggplot() +
  geom_boxplot(aes(x = model, y = value, fill = method), alpha = .3) +
  # geom_point(
  #   data= . %>% filter(model == "real_tables", !T_EUR), 
  #   aes(x = model, y = value, alpha = method), color = "#EB471E",
  #   shape = 4, position=position_jitterdodge(dodge.width=0.9, jitter.width = 0.2)
  # ) +
  ...
  guides(
    ...
    custom = guide_custom(
      gridtext::richtext_grob(
        paste(
          "<span style='color:#264DEB'>x</span> TRUE",
          "<span style='color:#EB471E'>x</span> FALSE",
          sep = "<br>"
        ),
        halign = 0,
