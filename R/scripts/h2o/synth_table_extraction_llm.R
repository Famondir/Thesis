...
df_characteristics <- df %>% filter(n_examples <= 5) %>% 
  left_join(
...
df_characteristics_unnested <- df_characteristics %>% 
  unnest(predictions) %>% 
  slice_sample(n = 100*sample_size)

df2 <- df_characteristics_unnested %>%
  mutate(
    .before = 1,
...
# Linear model
fit_lm_binomial <- h2o.glm(x = xvars, y = colnames(df_modeling_binomial_pdf)[1], training_frame = train, validation_frame = test)
