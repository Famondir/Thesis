...
df_synth <- readRDS("data_storage/real_table_extraction_extended_synth.rds")
df <- df_synth %>% left_join(units_real_tables)
...
# h2o.shutdown()
h2o.init(max_mem_size = "16G", port = 54326)
...
df_modeling_confidence_pdf <- df2 %>% select(all.vars(formula_confidence_pdf)) %>% 
  mutate(across(where(is.character), as.factor))
...
runtime_minutes <- floor(runtime_seconds / 60)
remaining_seconds <- runtime_seconds %% 60
cat("\033[1;32mRuntime after shp_lm_confidence plot:\033[0m", runtime_minutes, "minutes and", round(remaining_seconds), "seconds\n")
...
runtime_minutes <- floor(runtime_seconds / 60)
remaining_seconds <- runtime_seconds %% 60
cat("\033[1;32mRuntime after shp_rf_confidence plot:\033[0m", runtime_minutes, "minutes and", round(remaining_seconds), "seconds\n")
...
# runtime_minutes <- floor(runtime_seconds / 60)
# remaining_seconds <- runtime_seconds %% 60
# cat("\033[1;32mRuntime after shp_xgb_confidence plot:\033[0m", runtime_minutes, "minutes and", round(remaining_seconds), "seconds\n")
...
df2 <- df_characteristics %>%
  ...
  pivot_longer(
    ...
    .before = 1, 
    missing = if_else(year == "this_year", missing_this_year, missing_previous_year),
    confidence = if_else(year == "this_year", confidence_this_year, confidence_previous_year),
  ) %>% 
  group_by(match) %>% slice_sample(n = sample_size/2)
...
) %>% saveRDS(paste0("data_storage/h2o/real_table_extraction_synth_context_h2o_results_sample_",sample_size,"_shap_",test_sample_size,"_NA_recoded_extended.rds"))