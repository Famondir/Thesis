...
df <- readRDS("data_storage/table_extraction_regex.rds") %>% filter(
  table_type == "real_tables_extended"
)
...
table_characteristics <- read.csv("../benchmark_truth/real_tables_extended/table_characteristics_more_examples.csv") %>% 
  mutate(
    filepath = paste0("../../benchmark_truth/real_tables_extended/", company, "__", filename) %>% str_replace(".pdf$", ".csv")
  ) %>% as_tibble()
...
list(
  ...
  #   )
  # )
) %>% saveRDS(paste0("data_storage/h2o/real_table_extraction_regex_h2o_results_sample_",sample_size,"_shap_",test_sample_size,"_NA_recoded_extended.rds"))