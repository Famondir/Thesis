...
This chapter outlines the methodological framework adopted to address the research questions posed subsequent. It details the problem definition, research design and evaluation strategies used to investigate the effectiveness of \acr{LLM}s and alternative approaches for loacting financial information in and extracting it from German annual reports.

For each research question, specific hypotheses are formulated and the corresponding experimental setups are described. It describes our data strategy, including data selection, ground truth creation and preprocessing steps. The chapter also elaborates on the experimental framework, detailing the various approaches benchmarked, including rule-based methods and machine learning techniques.
...
The research design for this thesis is set up, following the guidelines found in @wohlinExperimentationSoftwareEngineering2024. Figure \@ref(fig:reasearch-design-image) shows the decisions made following these guidelines. According to @collisBusinessResearchPractical2014 research classification the outcome of this thesis is applied research, focusing on solving a practical problem. This thesis' purpose is evaluation research, comparing different approaches with each other (*benchmarking*). The data collected in our experiments is of quantitative nature and its evaluation uses (semi-)quantitative methods.
...
We refer to the third research question using the term *error rate guidance*.
...
Model specific predictors are the model family and the parameter size. Predictors related to the prompting strategies are related to in-context learning decisions, e.g. how many examples should be presented, how should those examples be chosen and is it permitted to use examples from the same company as the current subjects tasks document. Example for table characteristics investigated are the number of columns, visual separation and if there is a currency unit given — i.e. *T€*.

A list of all investigated predictors and their assumed directions of effect on multiple measures of the information extraction task can be found in the hypotheses evaluation tables in Appendix \@ref(feature-effect-analysis).
...
In a \acr{HITL} application the recall value might be of higher interest than the F1 score. More precisely, in those cases the number of pages to check until the correct page of interest is found. Thus, the top k recall is reported additionally, if the approach permits to rank the classified pages according to a score.
...
The population of annual reports of interest for the work at the \acr{RHvB} is composed of all annual reports of companies, where the state of Berlin holds a share. There are often multiple versions of those annual reports: one that is publicly available and one targeting share- and stakeholders. The structure and layout of these reports is quite heterogeneous. Often there is a second version that is used internally or for communications with public administrations. They often consist of plain text and tables and show neither diagrams nor photos.

Since the evaluations are run on the \acr{BHT} cluster and partially in the Azure cloud, we work with the publicly available reports, while at \acr{RHvB} the internal documents are processed. The annual reports mostly are downloaded from the companies websites. Some documents are accessed via [Bundesanzeiger](https://www.bundesanzeiger.de) or the [digitale Landesbibliothek Berlin](https://digital.zlb.de). We assume, that our findings can be transferred to the confidential reports, because the structure and content of the tables targeted in this thesis are almost identical. The visual differences should have little impact.

The page identification task classifies every page of the annual reports. The information extraction task only processes pages, which contain **Aktiva** tables. In addition, a set of self-generated synthetic **Aktiva** tables is used for the information extraction task. It is created to systematically investigate potential effects of characteristics financial tables could have.

### Sampling methodology

The reports are selected to ensure that the sample reflects the diversity of structure and content present in the overall population of documents.

::: paragraph-start
##### Page identification

We select companies from different fields of business for the page identification task and download all publicly available annual reports. The companies chosen are found in the first row of Figure \@ref(fig:beteiligungsunternehmen).
:::

Our assumption is, that each company within a branch can represent the document style for other companies of the same branch and that the position within a column has no implications for the annual reports styles or content. We additionally download reports for *GESOBAU AG*, because we realize that the *degewo AG* reports would require \acr{OCR} preprocessing. A description of the resulting data set can be found in Section \@ref(ground-truth-dataset-composition).

::: paragraph-start
##### Information extraction

For the information retrieval task we download all publicly available reports for most companies. We found no publicly available reports for half of the companies from the field of culture and some companies of the education sector and thus exclude these companies from our investigation. The amount of documents chosen for each company is more balanced than for the page identification task. A description of the resulting dataset can be found in Section \@ref(ground-truth-dataset-composition).
:::

We additionally exclude all companies, that do not report their assets table in the most detailed granularity defined by the legal text, because these asset tables do not match with the strict schema we use. This decision excludes the reports of well known companies as BVG and BSR.
...
The ground truth for both tasks is created by manual annotation through the authors. We use results of early experiments to check the ground truth for mistakes or missed items. We resolve found issues. Details on the ground truth creation processes can be found in Section \@ref(ground-truth-creation).

### Preprocessing

We use plain text extracted from the annual reports for all tasks. We do not extract geometric coordinates for the text. @auerDoclingTechnicalReport2024 describes, that available open-source PDF parsing libraries may show issues as poor extraction speed or randomly merged text cells. We test five PDF extraction libraries, because the results of all subsequent experiments will depend on the text extracts. Appendix \@ref(text-extraction-benchmark) shows the results.

We perform no manual data cleaning, because this will not be done from the employees of \acr{RHvB} neither.

### Data splitting

When we train a machine learning model, we split the data into train and a test set. We do not use a validation set, because we do not compare models using an extended hyper-parameter variation strategy. Instead we just report the performance found for the models build with default settings. We build two \acr{RF}s for the term frequency approach in the page identification task and additional ones for evaluating the hypotheses for the information extraction task.

Building the term frequency \acr{RF}, we face a highly imbalanced dataset. We apply undersampling for the training and evaluate the model on the imbalanced test set.
...
Table \@ref(tab:llm-overview) gives an overview on all \acr{LLM}s we use for the tasks in this thesis. It lists the passive parameter count in billions for each \acr{LLM} and shows in which specific approach it is used with a tick. We use `r length(unique(df_llm_overview$model))` models from `r length(unique(df_llm_overview$model_family))` model families. If available, we use a instruction fine-tuned version of the models.
...
We use normalized term frequencies and normalized float frequency as features for classification with a \acr{RF}. We use the predicted scores to build a ranking, for which page most probably contains the target pages. To handle the unbalanced data we use an undersampling approach for the training dataset creation. Figure \@ref(fig:tf-flowchart) visualizes, how the prediction works in this approach.
:::
...
We use \acr{LLM}s to extract the numeric values of synthetic **Aktiva** tables with restricted generation. The procedure is identical as with the real **Aktiva** tables. We extract all values with and without an explicit instruction on how to proceed with currency units. We limit our test on the open-weight models, because the number of synthetic tables is large and benchmarking closed-weight models is not the focus of this thesis.
:::

::: paragraph-start
##### Hybrid approach

We use \acr{LLM}s to extract the numeric values of real **Aktiva** tables with restricted generation, providing examples from synthetic **Aktiva** tables. The procedure is identical as with the real **Aktiva** tables. We extract all values with and without an explicit instruction on how to proceed with currency units. We limit our test on the open-weight models, because benchmarking closed-weight models is not the focus of this thesis.
:::

### Error analysis {#error-analysis-methodology}

To better understand the limitations of the evaluated models and prompting strategies and to find ways to further improve the system, we will conduct a detailed error analysis. Representative error cases will be documented to illustrate typical failure modes and to inform potential improvements for future work.

::: paragraph-start
##### Page identificaion

For page identification, the analysis will include investigation of misclassified pages using confusion matrices and error rate statistics to quantify types of misclassification.
:::

::: paragraph-start
##### Information extraction

For information extraction, error rates will be analyzed across different experimental setups, with stratified analysis by company, table characteristics (such as currency units and input format) and instruction specifications. We will investigate the errors unique for guided and restricted generation approaches.
:::

Based on the findings reported by @liExtractingFinancialData2023, we expect several characteristic error types to occur in the information extraction process. These include omissions, where the model fails to extract all values of a list; confusion between thousands and millions; and incorrect identification of rows and columns within tables.

### Evaluation methods

We use a lot of visual representations for the interpretation of the results. Many of those can be found in the technical report sections of the appendix. Bigger graphics with a lot of small multiples can be found in the appendix \@ref(figure-collection). These visuals have a high information density and enable us to compare a lot of results, including distributional information, at once[^03_methodology-1].

[^03_methodology-1]: A big screen with high resolution is helpful for this process.
...
We employ boxplots extensively to compare the distributions of sample results. During inspection, we also examine violin plots or add point jitter to the graphics to assess potential multi-modality. If interesting details are identified, we retain them in the presented graphics; otherwise, we present only the boxplots.
:::
...
In this chapter, we outlined the methodological framework for automated page identification and information extraction in financial documents. We formulated the problem defnitions and research questions, along with specific hypotheses to be tested.

We described the selection and preparation of datasets, the annotation strategies for ground truth creation and the design of benchmarking tasks. The chapter presented the approaches used for data processing, prompt engineering and evaluation metrics. We also presented the methods used for error analysis and interpretation of results.

Having established the methodological foundation, the next chapter focuses on the practical realization of these concepts. It presents the technical implementation, describes the computational environments and documents the software tools and workflows used to translate the proposed methodology into a functional system.
