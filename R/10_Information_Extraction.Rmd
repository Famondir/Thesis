...
# Appendix B - Information extraction report {#information-extraction-report}
...
This section presents the results for the table extraction task performed with \acr{LLM}s on **Aktiva** tables. Section \@ref(real-table-extraction-results) compraes the performance of open source models on real tables. It also compares those results with the table extraction performance achieved with models by OpenAI.

Section \@ref(synthetic-table-extraction) presents the results on synthetic **Aktiva** tables. Section \@ref(real-table-extraction-synth-context) shows a hybrid approach, where synthetic tables are used for the in-context learning, to extract real **Aktiva** tables. Finally, we summarize the results for all approaches in Section \@ref(comparing-table-extraction-methods).
...
```{r create-df-overview-gpt, echo=echo_flag, warning=warning_flag, message=message_flag, cache=TRUE, cache.extra = tools::md5sum(c("data_storage/real_table_extraction_llm.rds", "data_storage/real_table_extraction_azure.rds"))}
model_by_size_gpt <- c(
...
table <- df_overview_gpt %>% group_by(model, method) %>% 
  reframe(
  ...
  mutate(
    ...
    # median_runtime = round(median_runtime)
      ...
      "median correct total" = median_percentage_correct_total,
      "mean correct total" = mean_percentage_correct_total
      ) %>% 
  render_table(
    alignment = "llrr",
...
#   table <- table %>% column_spec(3, width = "3cm")
# }
...
This results in `r n_synth_table_extraction` files, that hold the results of 4_915 table extractions each. For the investigation of potential predictors influences the \acr{RF} is generated with a sample of 50_000 of these 68_810 results and finally, the \acr{SHAP} values are calculated with 2_000 rows of data.
