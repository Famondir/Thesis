...
Additionally we identified all pages, that contain any table for a previous sample, that is based on all the pages that are classified as target by the \acr{regex} approach. This ground truth is used to test the capabilities of various Python libraries, \acr{LLM}s and visual models to detect tables. Results can be found in the appendix in section \@ref(table-detection-benchmark).
