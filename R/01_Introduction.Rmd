...
For older sources, this approach is often the only alternative. For future information sharing, however, a more direct solution exists: providing data in a machine-readable format from the outset. Otherwise, we continue to face the "Last Mile Problem" [@liAddressingLastMile2023]^[The "Last Mile Problem" is a term
that originally symbolizes inefficiencies in the final stage of delivering goods or services to end-
users. @liAddressingLastMile2023 use it to describe the difficulties that stakeholders face when analyzing \acr{PDF}-type financial Open Government Data.]. Since the format of received data can only be changed for information one owns, users must cope with the formats provided until the need for machine-readable data is recognized and addressed by data owners.
