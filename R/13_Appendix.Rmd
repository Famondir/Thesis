...
frameworks as \acr{vllm} make it easy to deploy open-weight models for
...
time to ocr 106 images with pdf2image and pytesseract: 1:28+11:41
...
First experiments for the page identification task ran on a smaller dataset. The pages used for this setup are the pages, that are classified as target class by the regular expression approach. Without batch processing and the \acr{vllm} framework classifying these `r json_data$results %>% fromJSON() %>% nrow()` pages already takes very long. This motivated the test, if image detection models can be used for the page refinement by detecting tables. Section \@ref(yolo) shows the results of this attempt.
...
```{r df-yolo, echo=echo_flag, warning=warning_flag, message=message_flag}
df_table_detection_result %>% 
  select(model, runtime, pr_auc) %>% 
  mutate(pr_auc = format_floats(pr_auc, 3)) %>%
  render_table(alignment="lrr", caption="Comparing the runtime and AUC score of the precision-recall-curves for the three visual table detection models.", ref = opts_current$get("label"))
```


```{r pr-curve-yolo, echo=echo_flag, warning=warning_flag, message=message_flag, out.width="100%", fig.cap="Showing the precision-recall-curves for the three visual table detection models."}
...
```{r metric-plots-slider, echo=echo_flag, warning=warning_flag, message=message_flag, class.chunk='image-slider', out.width="100%", results='asis', eval=knitr::is_html_output()}
...
data_llm_speed <- data.frame(
 ...
 data_llm_speed %>% 
  render_table(
    alignment = "rrrr",
    caption = "Comparing time (in seconds) for extract the information from ten Aktiva tables using different libraries and approaches.",
    ref = opts_current$get("label")
  )
```
