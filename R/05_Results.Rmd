...
This chapter focuses on presenting results that answer whether an approach can be used to solve a task. Additional findings are discussed in Chapter \@ref(discussion). Further results are presented in the appendix, and we refer to the relevant appendix chapters documenting our investigation and the results of the sub-tasks at the beginning of each following section.
...
[^05_results-1]: Our personal opinion: Developing well-performing regular expressions can be very tedious and setting appropriate rules requires domain knowledge. It can be worth the effort if there are many documents with similar information to extract. For this thesis, it took several months. At least, there is now a pipeline that can be reused by exchanging the rules and keyword lists. Thus, the next similar task should be solved faster.
...
[^05_results-2]: In some cases, the information of interest spans two pages. These rare cases are not yet covered by the approaches presented here.

-   Section \@ref(regex-page-identification) presents the findings of a page range refinement using a list of key words with a regular expression.
...
We compare and summarize the results in Section \@ref(comparison-page-identification). Section \@ref(efficency-discussion) proposes an efficient combination of approaches to solve the task of this thesis and discusses its limitations.
...
We use normalized term frequencies and normalized float frequency as features for a classification with a \acrfull{RF}. The predicted scores are used to build a ranking, which page most probably contains the target pages. Undersampling is used during training, to handle the unbalanced data.
:::
...
The performance of approaches that do not utilize \acr{LLM}s is substantially lower. Only the term-frequency approach yields results that are suitable for downstream use, as it achieves a recall of 1.0. Table \@ref(tab:page-identification-performance-overview-top-k-recall) shows the results of the top k recall for the term-frequency and \acr{LLM} approaches. For the term-frequency approach the ranking is based on the scores of the \acr{RF} classifier. For the \acr{LLM}s the ranking is based on the calculated confidence scores.
...
(ref:information-extraction-intro-text) The second research question asks, how \acr{LLM}s can be used, to effectively extract specific information from a financial report. The task for this thesis is to extract the numeric values for the assets (*Aktiva*) table, which is part of the balance sheet (*Bilanz*). Hereafter, the German term **Aktiva** will be used. We are limiting the scope even further than in Section \@ref(page-identification-introduction), because it takes more time to manually create the first reference dataset.
...
Second, a dataset of 16_504 synthetic **Aktiva** tables is created. We generate these tables based on the extraction schema and fill them with random numeric values. Different table characteristics are systematically combined, to investigate potential effects of these features on the extraction performance. The tables are created as \acrfull{PDF}, \acrfull{HTML} and Markdown files each. This dataset allows to estimate the extraction performance, if there are no unknown row identifiers present.
...
On synthetic tables its mean performance is almost perfect, if currency units get respected. With \acr{HTML} documents we find 100 % correct predictions. With Markdown documents we find 99.9 % correct predictions as well. Figure \@ref(fig:comparing-table-extraction-performance-among-real-and-synth-aktiva-data) shows, that the better performance on the synthetic tables is found for almost all models.
...
Extracting the values from all 106 tables took Qwen3-235B around six minutes. Thus, excluding the setup time for the \acr{LLM}, Qwen3-235B-A22B-Instruct is around 100 times faster than a human.
:::
...
Section \@ref(page-identification-error-rate-results) presents the results found regarding our side research question for the page identification task. Section \@ref(information-extraction-error-rate-results) presents the results found regarding our side research question for the information extraction task. The detailed report can be found in the Appendix \@ref(error-guidance-report).
