...
-   Median percentage of correct predictions total:
    `r round(real_table_extraction_regex_total_performance_median, 3)`
-   Median percentage of correct numeric predictions:
    `r round(real_table_extraction_regex_num_performance_median, 3)`
-   Median of F1 score:
    `r round(real_table_extraction_regex_NA_F1_median, 3)`
...
df_two_header_lines = pd.read_html("../benchmark_results/table_extraction/hypotheses_and_results_real_table_extraction_regex.html", header = [1])
...
```{r costs-azure, echo=echo_flag, warning=warning_flag, message=message_flag, cache=TRUE, cache.extra = tools::md5sum("../CostManagement_master-thesis_2025.csv"), results='asis'}
...
```{r table-extraction-llm-errors-gpt, echo=echo_flag, warning=warning_flag, message=message_flag, out.width="100%", fig.cap="Showing the number of predictions OpenAI's models made."}
...
```{r table-extraction-llm-confidence-lm, echo=echo_flag, warning=warning_flag, message=message_flag, fig.height=5, out.width="100%", fig.cap="Estimating the relative frequency to find a wrong classification over different confidence intervals."}
...
at the same time. Synthetic data potential could help to answer such
questions.
...
```{r table-extraction-llm-respecting-units-plot, echo=echo_flag, warning=warning_flag, message=message_flag, out.width="100%", fig.cap="Comparing the numeric prediction performance for the hybrid approach, based on the fact, if the LLM is prompted to respect currency units."}
df_real_table_extraction_synth_respect_units %>% 
  ggplot() + 
...
confidence_vs_truth_hybrid %>% group_by(model, method, method_family, respect_units, predicted_NA) %>% reframe(median_conf = median(confidence)) %>% pivot_wider(names_from = respect_units, values_from = median_conf, names_prefix = "median_conf_units_") %>% # mutate(delta = units_FALSE - units_TRUE) %>% select(-c(units_FALSE, units_TRUE)) %>% pivot_wider(names_from = n_col_T_EUR, values_from = delta, names_prefix = "n_cols_T_EUR_") %>% group_by(model) %>% summarise_if(is.numeric, median) %>% 
  mutate(across(is.numeric, ~round(., 2))) %>% 
  render_table(
    alignment = "llllrr",
...
Thus, excluding the setup time for the LLM, Qwen3-235B-A22B-Instruct is
around 100 times faster than a human.
