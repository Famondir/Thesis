...
Figure \@ref(fig:table-dientification-llm-confidence-multi) summarizes
...
is top k accuracy. It is of interest which value of k is required to get
a recall of 100 %.
...
\@ref(fig:top-n-accuracy-term-frequency) shows the top n accuracy for
both random forests. On the left side the top n accuracy on the
imbalanced test dataset is shown. On the right side the performance on
the train dataset.
...
```{r top-n-accuracy-term-frequency, echo=echo_flag, warning=warning_flag, message=message_flag, fig.cap="Comparing the top n accuracy on training and test dataset among the random forest with two and four predictors."}
...
Baseline (exhaustive, high recall):

-   Aktiva = R: 0.997, P: 0.132, F1: 0.233

-   GuV = Recall: 1, precision: 0.210, F1: 0.350

-   Passiva = R: 0.993, P: 0.130, F1: 0.230

TOC (machine readable, higher precision)

-   Aktiva = R: 0.256, P: 0.600, F1: 0.359

-   GuV = R: 0.245, P: 0.446, F1: 0.316

-   Passiva = R: 0.210, P: 0.500, F1: 0.296

TOC (combi, higher recall)

-   Aktiva = R: 0.268, P: 0.338, F1: 0.299

-   GuV = R: 0.363, P: 0.378, F1: 0.370

-   Passiva = R: 0.222, P: 0.281, F1: 0.248
...
better[^05_results-23] on the synthetic dataset compared to the real
...
[^05_results-23]: A comparison of the numeric values over all methods
    can be found in section \@ref(comparing-table-extraction-methods).

-   a duplicated row name[^05_results-24]
-   numeric columns extracted separated from row names by extraction
    libraries
-   sums in the same row as the single values[^05_results-25]
-   with pdfium: missing white space[^05_results-26]
-   with pdfium: random line breaks[^05_results-27]

[^05_results-24]: The row *Geleistete Anzahlungen* can be found in two
    parts of the table and the simple approach just matches the numbers
    to the first found entry.

[^05_results-25]: In this case the \acr{regex} takes the sum as the
    value for the previous year.

[^05_results-26]: This can form unexpected numeric patterns or prevent
    the row names to be recognized.

[^05_results-27]: The approach takes care of line breaks between words,
...
have been benchmarked[^05_results-28]. The results are presented in
...
[^05_results-28]: The models *deepseek-ai_DeepSeek-R1-Distill-Qwen-32B*
...
without any guidance[^05_results-29] (see Table
...
[^05_results-29]: There is an external guidance through the provided
...
best with three examples[^05_results-30].

[^05_results-30]: Phi4 also perfroms best with three examples. But this
...
approaches[^05_results-31]. With a gpt-oss-120b instance hosted on Azure
the guided decoding worked flawless.

[^05_results-31]: With the static example approach there have been 24
...
columns[^05_results-32]. Except for Llama 3.1 all models achieved much
...
[^05_results-32]: Synthetic data is used here because the
