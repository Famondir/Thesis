...
top_performer_binary %>% 
  render_table(alignment="llllrrr", caption="Overview of benchmarked LLMs for the classification tasks. Limiting the number of examples provided for the few shot approach to 3.", ref = opts_current$get("label"))
```
...
```{r def-micro-pr-function, echo=echo_flag, warning=warning_flag, message=message_flag}
library(yardstick)
...
df_temp_mistral <- df_binary %>% calc_micro_f1(model, method)
...
Figure \@ref(fig:table-dientification-llm-confidence) summarizes the
...
can be fully trusted. These empirical intervals will probably get
smaller, once more data is evaluated. If one is less strict and accepts
misclassifiaction rates of 1 % the found interval for **Passiva** starts
at 0.8 and is probably less depended on the sample evaluated.

For Qwen3-8B we find almost no range without any misclassification. One
could build an application that is not asking for a human intervention
for reported confidences over 0.9 and then give the possibility to
change the page to extract information from later on. Anyhow, only a few
predictions for both models have such a high confidence score. Thus, a
human will double check the selected pages in most cases.

```{r table-dientification-llm-confidence, echo=echo_flag, warning=warning_flag, message=message_flag, out.width="100%", fig.cap="Comparing the reported confidence scores for the page identification task for the Mistral and Qwen 3 with 8B parameters.", cache=TRUE, cache.extra = tools::md5sum("data_storage/data_storage/page_identification_llm.rds"), fig.height=3}
...
confidence_vs_truth %>% rename(confidence = confidence_score) %>% 
  ...
  ggplot(aes(x = conf_center, y = chance_false, color = chance_low#, fill = chance_low
             )) +
  geom_col(alpha = 0.7) +
  ...
  labs(x = "Confidence Interval", y = "Chance False (%)", color = "mistake rate") +
  coord_cartesian(
    # ylim = c(0, 100), 
    xlim = c(0,1)
    ) +
...
bigger models are better with the \\acr{mcc} task Llama-4-Scout almost
perfect F1 for all classes
...
missclassified as Aktiva No flipped confidence [^05_results-19]

[^05_results-19]: classify framework in needs special models with
...
better[^05_results-20] on the synthetic dataset compared to the real
...
[^05_results-20]: A comparison of the numeric values over all methods
    can be found in section \@ref(comparing-table-extraction-methods).

-   a duplicated row name[^05_results-21]
-   numeric columns extracted separated from row names by extraction
    libraries
-   sums in the same row as the single values[^05_results-22]
-   with pdfium: missing white space[^05_results-23]
-   with pdfium: random line breaks[^05_results-24]

[^05_results-21]: The row *Geleistete Anzahlungen* can be found in two
    parts of the table and the simple approach just matches the numbers
    to the first found entry.

[^05_results-22]: In this case the \acr{regex} takes the sum as the
    value for the previous year.

[^05_results-23]: This can form unexpected numeric patterns or prevent
    the row names to be recognized.

[^05_results-24]: The approach takes care of line breaks between words,
...
have been benchmarked[^05_results-25]. The results are presented in
...
[^05_results-25]: The models *deepseek-ai_DeepSeek-R1-Distill-Qwen-32B*
...
without any guidance[^05_results-26] (see Table
...
[^05_results-26]: There is an external guidance through the provided
...
best with three examples[^05_results-27].

[^05_results-27]: Phi4 also perfroms best with three examples. But this
...
approaches[^05_results-28]. With a gpt-oss-120b instance hosted on Azure
the guided decoding worked flawless.

[^05_results-28]: With the static example approach there have been 24
...
columns[^05_results-29]. Except for Llama 3.1 all models achieved much
...
[^05_results-29]: Synthetic data is used here because the
