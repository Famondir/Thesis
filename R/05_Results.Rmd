...
df_binary %>% left_join(df_classification_n_examples, by = c(
  ...
  filter(model == "mistralai_Ministral-8B-Instruct-2410", loop < 2) %>% 
...
df_filtered <- df_binary %>% filter(
  classification_type == "Aktiva", n_examples<=3
  ) %>%
  arrange(desc(f1_score))
...
precision-recall-curve for the best performing model twice. On the left
plot the line color represents the threshold score one could use to
decide when to accept a response as it is. On the right plot the line
color is showing the F1 score that results with a chosen threshold. The
precision recall curves for all target classes look identical. Thus, a
combined micro average curve is presented. For an almost perfect model
there is nothing one can adjust. The curve shown in Figure
\@ref(fig:micro-pr-curve-llm-binary-qwen-1-5b) is more interesting.
Here, one can decide if the F1 score or the recall score is more
important for the workflow one wants to implement.
...
model <- "mistralai_Ministral-8B-Instruct-2410"
...
p_aktiva / p_passiva / p_guv +
  plot_annotation(title = str_c(model, " with ", method))
```

```{r micro-pr-curve-llm-binary-qwen-1-5b, echo=echo_flag, warning=warning_flag, message=message_flag, out.width="100%", fig.height=4, fig.cap="Showing the precsion-recall-curve for a ."}
df_temp_qwen <- df_binary %>% calc_micro_f1("Qwen_Qwen2.5-1.5B-Instruct", "1_rag_examples")
df_temp_qwen %>% plot_pr_double_curve("Passiva")
```
...
missclassified as Aktiva No flipped confidence [^05_results-18]

[^05_results-18]: classify framework in needs special models with
...
better[^05_results-19] on the synthetic dataset compared to the real
...
[^05_results-19]: A comparison of the numeric values over all methods
    can be found in section \@ref(comparing-table-extraction-methods).

-   a duplicated row name[^05_results-20]
-   numeric columns extracted separated from row names by extraction
    libraries
-   sums in the same row as the single values[^05_results-21]
-   with pdfium: missing white space[^05_results-22]
-   with pdfium: random line breaks[^05_results-23]

[^05_results-20]: The row *Geleistete Anzahlungen* can be found in two
    parts of the table and the simple approach just matches the numbers
    to the first found entry.

[^05_results-21]: In this case the \acr{regex} takes the sum as the
    value for the previous year.

[^05_results-22]: This can form unexpected numeric patterns or prevent
    the row names to be recognized.

[^05_results-23]: The approach takes care of line breaks between words,
...
have been benchmarked[^05_results-24]. The results are presented in
...
[^05_results-24]: The models *deepseek-ai_DeepSeek-R1-Distill-Qwen-32B*
...
without any guidance[^05_results-25] (see Table
...
[^05_results-25]: There is an external guidance through the provided
...
best with three examples[^05_results-26].

[^05_results-26]: Phi4 also perfroms best with three examples. But this
...
approaches[^05_results-27]. With a gpt-oss-120b instance hosted on Azure
the guided decoding worked flawless.

[^05_results-27]: With the static example approach there have been 24
...
columns[^05_results-28]. Except for Llama 3.1 all models achieved much
...
[^05_results-28]: Synthetic data is used here because the
