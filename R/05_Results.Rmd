...
was detected. Figure \@ref(fig:table-extraction-doc-base) shows how many
**Aktiva** tables are used for all tasks in this subsection, that use
...
```{r table-extraction-doc-base, echo=echo_flag, warning=warning_flag, message=message_flag, out.width="100%", fig.cap="Showing the number of documents used for the table extraction task. The number of Aktiva tables is equal to the number documents."}
...
Figure \@ref(fig:\@ref(fig:table-extraction-regex-performance)) shows,
...
median performance of the \acr{regex} approaches will be reflected by a
dashed line in the box-plots in subsequent sections.

```{r table-extraction-regex-performance, echo=echo_flag, warning=warning_flag, message=message_flag, out.width="100%", fig.cap="Performance overall and on numeric value extraction with regular expressions."}
...
We find, that it is worth mentioning, that

-   the visual separation of columns,

-   presenting sums in the rows with higher hierarchy row labels and

-   having T€ in the previous year column

have a negative effect on the F1 score. The mean \acr{SHAP} values for
these observations are almost 0.03 and higher (on a scale that ranges
from 0 to 1). The distribution for *T_in_previous_year* and
*vis_separated_cols* are quite balanced. The findings for the balanced
chracteristics can be seen as a reason to change the corresponding
hypotheses for future investigations.

The hypothesis about a positive influence of a missing of a value for
the binomial prediction gets solid support with a mean absolute
\acr{SHAP} value of over
...
The visual results for the \acr{SHAP} analysis on the synthetic dataset
brought some insides into the question, under which condition the two
PDF extraction libraries perform differently. These results can be
treated as reliable since the model has been trained with 24_576 table
extracts and the \acr{SHAP} values have been calculated on 2_000
examples each.

The number of columns is having an opposite effect for the two libraries
(see Figure
\@ref(fig:synth-table-extraction-regex-shap-num-details-plot) A).
Overall the effect is positive. So more colums give better results.

Besides that we find, that *pdfium* is struggling with some of the table
...
this exemplary for the characteristic *header_span*.
...
is cause for the faulty text extractions of *pdfium*.
...
table extraction performance achieved with models by OpenAI. Subsection
\@ref(synthetic-table-extraction) presents the results on synthetic
**Aktiva** tables. Subsection \@ref(real-table-extraction-synth-context)
shows a hybrid approach, where synthetic tables are used for the
in-context learning, to extract real **Aktiva** tables. Finally, we
summarize the results for all approaches in subsection
\@ref(comparing-table-extraction-methods).
...
have been benchmarked[^05_results-26]. The results are presented in
...
without any guidance[^05_results-27] (see Table
...
Table \@ref(tab:table-metrics-llm-real-table-extraction) shows the
median performance for the best model-method approach for each model
family. Most of the top performing model-method combinations rely on the
maximum number of examples provided. Only the Llama-3 and Falcon3 model
perform best with three examples[^05_results-28].

[^05_results-28]: Phi4 also perfroms best with three examples. But this
...
approaches[^05_results-29]. With a gpt-oss-120b instance hosted on Azure
the guided decoding worked flawless.

[^05_results-29]: With the static example approach there have been 24
...
df_overview_gpt %>% group_by(model, method) %>% summarise(median_percentage_correct_total = median(percentage_correct_total)) %>% arrange(desc(median_percentage_correct_total)) %>% slice_head(n = 1) %>% ungroup() %>% arrange(desc(median_percentage_correct_total)) %>% mutate(median_percentage_correct_total = round(median_percentage_correct_total, 2)) %>% rename("median correct total" = median_percentage_correct_total) %>% kbl()
```
...
token_prop <- df_real_table_extraction %>% group_by(model, method, n_examples) %>% summarize(
    ...
    summarize(median = median(request_tokens_total, na.rm = TRUE)) %>% mutate(five_examples = n_examples == 5) %>% group_by(five_examples) %>% summarise(sum = sum(median))
...
df_synth_top_performance <- df_synth_table_extraction %>% group_by(model, method) %>% mutate(median_total = median(percentage_correct_total, na.rm = TRUE)) %>%
  arrange(desc(median_total)) %>% group_by(model_family) %>% slice_head(n = 1) %>% 
  select(model_family, model, method_family, n_examples, median_total) %>% arrange(desc(median_total)) %>% 
  mutate(median_total = round(median_total, 3))

df_synth_top_performance_small <- df_synth_table_extraction %>% group_by(model, method) %>% filter(parameter_count < 17) %>% mutate(median_total = median(percentage_correct_total, na.rm = TRUE)) %>%
  arrange(desc(median_total)) %>% group_by(model_family) %>% slice_head(n = 1) %>% 
  select(model_family, model, method_family, n_examples, median_total) %>% arrange(desc(median_total)) %>% 
  mutate(median_total = round(median_total, 3))

n_better_as_regex <- df_synth_table_extraction %>% group_by(model, method) %>% summarise(median_total = median(percentage_correct_total, na.rm = TRUE)) %>%
...
`r synth_table_extraction_regex_total_performance_median`.
...
df_synth_top_performance %>% 
  ...
  render_table(caption = "Comparing best median table extraction performance with synthetic 'Aktiva' dataset for each model family", ref = opts_current$get("label"), alignment = "lllrr", dom="t")
```
...
best_models_with_examples <- df_real_table_extraction_synth %>% filter(str_detect(method_family, "n_")) %>% group_by(model, method) %>% summarise(median_synth = median(percentage_correct_total)) %>% arrange(desc(median_synth)) %>% slice_head(n = 1) %>% ungroup() %>% arrange(desc(median_synth)) %>% left_join(
  df_real_table_extraction %>% group_by(model, method) %>% summarise(median_real = median(percentage_correct_total))) %>% 
  left_join(
    df_real_table_extraction %>% group_by(model, method) %>% summarise(median_zero_shot = median(percentage_correct_total)) %>% filter(method == "zero_shot") %>% select(-method)
  ) %>% 
...
columns[^05_results-30]. Except for Llama 3.1 all models achieved much
...
[^05_results-30]: Synthetic data is used here because the
...
# df_real_table_extraction_synth_respect_units %>% group_by(model, method, respect_units, n_col_T_EUR) %>% reframe(median_num = median(percentage_correct_numeric)) %>% pivot_wider(names_from = respect_units, values_from = median_num, names_prefix = "units_") %>% mutate(delta = units_FALSE - units_TRUE) %>% select(-c(units_FALSE, units_TRUE)) %>% ggplot() + geom_histogram(aes(x = delta)) + facet_grid(model~n_col_T_EUR)
```
...
confidence_vs_truth %>% group_by(model, method, method_family, respect_units, predicted_NA) %>% reframe(median_conf = median(confidence)) %>% pivot_wider(names_from = respect_units, values_from = median_conf, names_prefix = "median_conf_units_") %>% # mutate(delta = units_FALSE - units_TRUE) %>% select(-c(units_FALSE, units_TRUE)) %>% pivot_wider(names_from = n_col_T_EUR, values_from = delta, names_prefix = "n_cols_T_EUR_") %>% group_by(model) %>% summarise_if(is.numeric, median) %>% 
