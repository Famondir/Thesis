...
class TableTemplateFillingExtractor(Extractor):
    ...
    def __get_static_example_message(self, markdown = False, **kwargs):
        system_messages = []
        ...
        if markdown:
            text_example = self.replace_special_characters(df_example.dropna(subset=[df_example.columns[-1]]).to_markdown(index=False))
        else:
