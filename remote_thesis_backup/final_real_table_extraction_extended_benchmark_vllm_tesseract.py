...
def main(model_name, extractor_type, test_run, n_loops, verbose, combine_system_prompts, no_think, batched, tensor_parallel_size, temperature, enforce_eager):
    ...
    embeddings = np.load("/pvc/thesis_benchmarks/embeddings_real_tables_extraction_job_tesseract.npy")

    with open("/pvc/thesis_benchmarks/pdf_texts_real_tables_extraction_job_tesseract.json", "r", encoding="utf-8") as f:
    ...
    collection = client.get_or_create_collection("real_tables_extended_tesseract"#, embedding_function=sentence_transformer_ef
    )
    ...
    for i in range(n_loops):
        ...
        if batched:
            benchmark.extract_tables_queued(sample, result_dir="/pvc/benchmark_results/table_extraction/llm/final/real_tables_more_examples_tesseract/" + model_name.replace("/", "_") +"_vllm"+ f"__benchmark_{extractor_type}"+f"__temperature_{temperature}"+("_test" if test_run else "")+f"__zero_shot__loop_{i}", no_think=no_think)
            benchmark.extract_tables_queued(sample, static_example=True, result_dir="/pvc/benchmark_results/table_extraction/llm/final/real_tables_more_examples_tesseract/" + model_name.replace("/", "_") +"_vllm"+ f"__benchmark_{extractor_type}"+f"__temperature_{temperature}"+("_test" if test_run else "")+f"__static_example__loop_{i}", no_think=no_think)
            for n_examples in [
                1, 3, 5
                ]:
                print(f"Running benchmark with n_examples={n_examples}")
                benchmark.extract_tables_queued(sample, random_examples=True, n_examples=n_examples, all_entries=entries, result_dir="/pvc/benchmark_results/table_extraction/llm/final/real_tables_more_examples_tesseract/" + model_name.replace("/", "_") +"_vllm"+ f"__benchmark_{extractor_type}"+f"__temperature_{temperature}"+("_test" if test_run else "")+f"__{n_examples}_random_examples__loop_{i}", no_think=no_think)
                for out_of_sample in [False, True]:
                    print(f"Running benchmark with out_of_sample={out_of_sample}")
                    benchmark.extract_tables_queued(sample, top_n_rag_examples=True, n_examples=n_examples, collection=collection, out_of_sample=out_of_sample, result_dir="/pvc/benchmark_results/table_extraction/llm/final/real_tables_more_examples_tesseract/" + model_name.replace("/", "_") +"_vllm"+ f"__benchmark_{extractor_type}"+f"__temperature_{temperature}"+("_test" if test_run else "")+f"__top_{n_examples}_rag_examples"+("_out_of_sample" if out_of_sample else "")+f"__loop_{i}", no_think=no_think)
        # else: 
