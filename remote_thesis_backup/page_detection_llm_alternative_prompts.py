...
phrase_dict = {
        ...
        "othertable": "a table of another type",
        "notable": "a text that does not contain a table"
    }

def get_random_example_message(all_entries, n_guv=1, n_activa=1, n_passiva=1, n_othertable=1, n_notable=1):
    import random
...
def get_rag_example_messages(text, collection, filepath, exclude_company="", embedding=None, report_distance=True, n_guv=1, n_activa=1, n_passiva=1, n_othertable=1, n_notable=1):
    examples = {key: [] for key in phrase_dict.keys()}
    system_messages = []
...
class BinaryStringClassifier(StringClassifier):
    ...
    def __get_random_example_message(self, **kwargs):
        return get_random_example_message(all_entries=kwargs.get("all_entries", []), n_guv=3, n_activa=1, n_passiva=1, n_othertable=1, n_notable=1)
...
class FiveClassStringClassifier(StringClassifier):
    ...
    def __get_random_example_message(self, **kwargs):
        return get_random_example_message(all_entries=kwargs.get("all_entries", []), n_guv=2, n_activa=2, n_passiva=2, n_othertable=2, n_notable=2)
    ...
    def get_messages(self, text, law_context = False, random_examples = False, rag_examples = False, top_n_rag_examples = False, **kwargs):
        messages = [
            ...
            4) Wenn der Text eine Tabelle enthält, die etwas anderes zeigt, antworte mit 'othertable'.\n\n
...
class PageDetectionBenchmarkLLM:
    def __init__(self, classifier, classifier_type, verbose=False):
    ...
    def classify_pages(self, entries, result_dir, out_of_sample = True, **kwargs):
        ...
        if self.classifier_type == "binary":
            ...
            df['match'] = ((df['type'] == 'GuV') & (df['predicted_type'] == 'GuV')) | ((df['type'] != 'GuV') & (df['predicted_type'] != 'GuV'))
