...
class TableTemplateFillingExtractor(Extractor):
    ...
    def get_json_string(self, df, multiplier=1):
        df_rounded = df.copy()
        df_rounded = df_rounded.map(lambda x: self.replace_special_characters(x) if isinstance(x, str) else x)
    ...
    def __get_random_example_message(self, all_entries, n_examples=3, **kwargs):
        import random
        ignore_units = kwargs.get('ignore_units', True)
        ...
        solutions = []
        for entry in sample:
            entry_filepath = entry.get('filepath', 'unknown').replace('.pdf', '.csv').replace('.html', '.csv').replace('.md', '.csv')
            df = pd.read_csv(entry_filepath)

            if ("real_tables" in entry_filepath):
                ...
                if not ignore_units:
                    df = df.map(lambda x: round(x/multiplier, 2)*multiplier if isinstance(x, (int, float)) else x)
                    # rounds values matching with the one in the pdfs (synthetic tables important)
                    multiplier = 1
    ...
    def __get_top_n_rag_example_message(self, text, collection, filepath, exclude_company="", embedding=None, report_distance=True, n_examples=5, **kwargs):
        ...
        for idx, (example, meta_data, distance) in enumerate(zip(examples['documents'][0], examples['metadatas'][0], examples['distances'][0])):
            entry_filepath = meta_data.get('filepath', 'unknown').replace('.pdf', '.csv').replace('.html', '.csv').replace('.md', '.csv')
...
{self.get_json_string(df_example)}
    ...
    def get_messages(self, text, static_example = False, random_examples=False, top_n_rag_examples = False, combine_system_prompts= False, **kwargs):
        ...
        messages = [
        {"role": "system", "content": "[Role and Context]: You are a helpful assistant that extracts information from tables."}
        ]
        ...
        baseprompt = f"""
            [Task]: Extract the information from the given table as JSON list.

            [Rules]:
            1) Each row should be an entry with five keys. The keys names are "E1", "E2", "E3", "year", "previous_year".
            2) The entries for "E1", "E2" and "E3" are given by an EBNF. You just have to extract the numeric values for "year" and "previous_year" from the second and third or fourth and fifth columns, respectively.
            3) If there are no corresponding numeric values for a given triple of "E1", "E2" and "E3" it should be represented by "null".
            4) Skip the first one or two rows if they contain headers or units.
            {('5) Sometimes there units are present in the table - either given for the whole table or individually per row. For example "TEUR" for thousand euros or "Mio. €." for million euros. Convert the extracted numeric values to euros by multiplying them with an appropriate factor. Here is an example list of units and their factors: '+str(unit_list).replace("\'", '"')) if not ignore_units else ''}
            [Table]: This is the table you should extract the information from:\n
            \n\n'''\n{text}\n''''
            """
        ...
        messages.append({"role": "user", "content": baseprompt})
...
class TableExtractionBenchmarkLLM():
    ...
    def evaluate_single_result(self, result, entry, ignore_units=True):
        evaluation = {
            'json_error': False,
            'grammar_error': False,
        }
        ...
        entry_filepath = entry.get('filepath', 'unknown').replace('.pdf', '.csv').replace('.html', '.csv').replace('.md', '.csv')
        df = pd.read_csv(entry_filepath)
        # df = df.map(lambda x: self.extractor.replace_special_characters(x) if isinstance(x, str) else x)
    ...
    def extract_tables_queued(self, entries, result_dir, out_of_sample = True, **kwargs):
        ...
        for entry in entries:
            text = entry['text'][0] if isinstance(entry['text'], list) else entry['text']
            text = self.extractor.replace_special_characters(text)
            # print(text)
        ...
        for result, entry, confidence, text in zip(results, entries, confidence_scores, texts):
            evaluation = self.evaluate_single_result(result, entry, ignore_units=kwargs.get('ignore_units', True))
