...
def main(model_name, extractor_type, test_run, n_loops, verbose, combine_system_prompts, no_think, batched, tensor_parallel_size, temperature, enforce_eager):
    ...
    embeddings = np.load("/pvc/thesis_benchmarks/embeddings_real_tables_extraction_job_pymupdf.npy")

    with open("/pvc/thesis_benchmarks/pdf_texts_real_tables_extraction_job_pymupdf.json", "r", encoding="utf-8") as f:
    ...
    collection = client.get_or_create_collection("real_tables_extended_pymupdf"#, embedding_function=sentence_transformer_ef
    )
    ...
    for i in range(n_loops):
        ...
        if batched:
            benchmark.extract_tables_queued(sample, result_dir="/pvc/benchmark_results/table_extraction/llm/final/real_tables_more_examples_pymupdf/" + model_name.replace("/", "_") +"_vllm"+ f"__benchmark_{extractor_type}"+f"__temperature_{temperature}"+("_test" if test_run else "")+f"__zero_shot__loop_{i}", no_think=no_think)
