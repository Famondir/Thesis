...
def main(model_name, extractor_type, test_run, n_loops, verbose, combine_system_prompts, no_think, batched, tensor_parallel_size, temperature):
    ...
    embeddings = np.load("/pvc/thesis_benchmarks/embeddings_synthetic_tables_markdown.npy")

    with open("/pvc/thesis_benchmarks/markdown_texts_synthetic_tables.json", "r", encoding="utf-8") as f:
    ...
    collection = client.get_or_create_collection("synthetic_tables_markdown", embedding_function=sentence_transformer_ef)
    ...
    for i in range(n_loops):
        ...
        if batched:
            benchmark.extract_tables_queued(sample, result_dir="/pvc/benchmark_results/table_extraction/llm/synth_tables_markdown/" + model_name.replace("/", "_") +"_vllm"+ f"__benchmark_{extractor_type}"+f"__temperature_{temperature}"+("_test" if test_run else "")+f"__zero_shot__loop_{i}", no_think=no_think)
            benchmark.extract_tables_queued(sample, markdown=True, static_example=True, result_dir="/pvc/benchmark_results/table_extraction/llm/synth_tables_markdown/" + model_name.replace("/", "_") +"_vllm"+ f"__benchmark_{extractor_type}"+f"__temperature_{temperature}"+("_test" if test_run else "")+f"__static_example__loop_{i}", no_think=no_think)
            benchmark.extract_tables_queued(sample, random_examples=True, n_examples=3, all_entries=entries, result_dir="/pvc/benchmark_results/table_extraction/llm/synth_tables_markdown/" + model_name.replace("/", "_") +"_vllm"+ f"__benchmark_{extractor_type}"+f"__temperature_{temperature}"+("_test" if test_run else "")+f"__random_examples__loop_{i}", no_think=no_think)
            benchmark.extract_tables_queued(sample, top_n_rag_examples=True, n_examples=3, collection=collection, result_dir="/pvc/benchmark_results/table_extraction/llm/synth_tables_markdown/" + model_name.replace("/", "_") +"_vllm"+ f"__benchmark_{extractor_type}"+f"__temperature_{temperature}"+("_test" if test_run else "")+f"__top_n_rag_examples__loop_{i}", no_think=no_think)
        else: 
            benchmark.extract_tables(sample, static_example=True, result_dir="/pvc/benchmark_results/table_extraction/llm/synth_tables_markdown/" + model_name.replace("/", "_") +"_vllm"+ f"__benchmark_{extractor_type}"+f"__temperature_{temperature}"+("_test" if test_run else "")+f"__static_example__loop_{i}", no_think=no_think)
