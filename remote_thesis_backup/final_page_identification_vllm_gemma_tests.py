...
class BinaryStringClassifier(StringClassifier):
    ...
    def get_messages(self, text, classification_type='GuV', law_context = False, random_examples = False, rag_examples = False, top_n_rag_examples = False, combine_system_prompts = False, **kwargs):
        ...
        if combine_system_prompts:
            ...
            if system_contents:
                ...
                messages.insert(0, {"role": "system", "content": "<|im_start|>system\n"+combined_content+"<|im_end|>"})

        messages.append({"role": "user", "content": f"<|im_start|>user\n[Text]: Here is the text to classify: \n\n'''\n{text}\n'''\n\n[Task]: Decide if the given text contains {phrase_dict[classification_type]}.\n\n[Rule]: Answer with 'yes' if it does. Otherwise answer with 'no'. Don't answer anything else thy 'yes' or 'no' and don't repeat the question.<|im_end|>"})
        return messages
    ...
    def classify_batched(self, texts, **kwargs):
        ...
        sampling_params = SamplingParams(
            ...
            # max_tokens=1,  # Only need one token for binary classification
            )
...
class FourClassStringClassifier(StringClassifier):
    ...
    def get_messages(self, text, law_context = False, random_examples = False, rag_examples = False, top_n_rag_examples = False, combine_system_prompts = False, **kwargs):
        ...
        #     2) If the text contains an 'Aktiva' (assets) table, answer with 'Aktiva'.\n\n
        #     3) If the text contains a 'Passiva' (liabilities) table, answer with 'Passiva'.\n\n
        #     4) If the text contains a table that is not an 'Aktiva' (assets), 'Passiva' (liabilities) or 'Gewinn- und Verlustrechnung' (GuV) table, answer with 'other table'.\n\n
        #     5) If the text contains something else, answer with 'no table'.\n\n
    ...
    def classify_batched(self, texts, **kwargs):
        ...
        #         # print(f"Token ID: {token_id}, Logit: {logits[i]}")
        #         if token_id not in [
        #             self.tokenizer.encode("no", add_special_tokens=False)[0],
        #             self.tokenizer.encode("oth", add_special_tokens=False)[0],
        #             self.tokenizer.encode("Ak", add_special_tokens=False)[0],
        #             self.tokenizer.encode("Pas", add_special_tokens=False)[0],
        #             self.tokenizer.encode("Gu", add_special_tokens=False)[0]
        #         ]:
        #             logits[i] = -np.inf
        #     return logits
        ...
        sampling_params = SamplingParams(
            ...
            # max_tokens=1,  # Only need one token for classification
        )
