...
from final_real_table_extraction_vllm import TableTemplateFillingExtractor, TableExtractionBenchmarkLLM
    ...
    for i in range(n_loops):
        ...
        if batched:
            ...
            for n_examples in [
                ...
                for out_of_sample in [False]:
                    print(f"Running benchmark with out_of_sample={out_of_sample}")
                    benchmark.extract_tables_queued(sample, top_n_rag_examples=True, n_examples=n_examples, collection=collection, out_of_sample=out_of_sample, benchmark=False, result_dir="/pvc/extraction_job/results/" + model_name.replace("/", "_") +"_vllm"+ f"__benchmark_{extractor_type}"+f"__temperature_{temperature}"+("_test" if test_run else "")+f"__top_{n_examples}_rag_examples"+("_out_of_sample" if out_of_sample else "")+f"__loop_{i}", no_think=no_think)
        # else: 
