...
class FiveClassStringClassifier(StringClassifier):
    ...
    def classify(self, text, **kwargs):
        ...
        guided_decoding_params = GuidedDecodingParams(choice=["Aktiva", "Passiva", "GuV", "othertable", "notable"],)
        sampling_params = SamplingParams(guided_decoding=guided_decoding_params)
...
class PageDetectionBenchmarkLLM:
    ...
    def classify_pages(self, entries, result_dir, out_of_sample = True, **kwargs):
        ...
        for entry in entries:
            ...
            predicted_class = self.classifier.classify(text = text, exclude_company = entry['company'] if out_of_sample else "", filepath=entry['filepath'], combine_system_prompts=self.combine_system_prompts, **kwargs)
    ...
    def classify_pages_batched(self, entries, result_dir, out_of_sample = True, **kwargs):
        ...
        for entry in entries:
            text = self.clean_text(entry['text'])
            kwargs['embedding'] = entry['embedding']
            texts.append(self.classifier.prepare_to_detect(text, combine_system_prompts=self.combine_system_prompts, **kwargs))

        # print(texts[0])

        results = self.classifier.classify_batched(texts, **kwargs)
        # print(f"Results: {results}")

        # for i, row in enumerate(predictions):
        #     predictions[i]['predicted_type'] = results[i]
