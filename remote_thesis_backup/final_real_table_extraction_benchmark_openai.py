...
from final_real_table_extraction_openai import TableTemplateFillingExtractor, TableExtractionBenchmarkLLM
...
def main(model_name, extractor_type, test_run, n_loops, verbose, combine_system_prompts, no_think, batched, tensor_parallel_size, temperature, enforce_eager):
    ...
    endpoint = "https://s9281-mdmylxmx-eastus2.cognitiveservices.azure.com/"
    model_name = model_name
    deployment = model_name

    subscription_key = "Fsh2F96yNeA7aQgddRMT5YncB2RkUkKiYN81wt2gLx0JJNTnZtc9JQQJ99BGACHYHv6XJ3w3AAAAACOGdpaF"
    ...
    if test_run:
        sample = entries[0:2]  # Use first 10 entries for test run
    else:
        sample = entries
    ...
    for i in range(n_loops):
        ...
        if batched:
            pass
        #     benchmark.extract_tables_queued(sample, result_dir="/pvc/benchmark_results/table_extraction/llm/final/real_tables/" + model_name.replace("/", "_") +"_vllm"+ f"__benchmark_{extractor_type}"+f"__temperature_{temperature}"+("_test" if test_run else "")+f"__zero_shot__loop_{i}", no_think=no_think)
            # benchmark.extract_tables_queued(sample, static_example=True, result_dir="/pvc/benchmark_results/table_extraction/llm/final/real_tables/" + model_name.replace("/", "_") +"_vllm"+ f"__benchmark_{extractor_type}"+f"__temperature_{temperature}"+("_test" if test_run else "")+f"__static_example__loop_{i}", no_think=no_think)
            # for n_examples in [
            #     1, 3, 5
            #     ]:
            #     print(f"Running benchmark with n_examples={n_examples}")
            #     benchmark.extract_tables_queued(sample, random_examples=True, n_examples=n_examples, all_entries=entries, result_dir="/pvc/benchmark_results/table_extraction/llm/final/real_tables/" + model_name.replace("/", "_") +"_vllm"+ f"__benchmark_{extractor_type}"+f"__temperature_{temperature}"+("_test" if test_run else "")+f"__{n_examples}_random_examples__loop_{i}", no_think=no_think)
            #     for out_of_sample in [False, True]:
            #         print(f"Running benchmark with out_of_sample={out_of_sample}")
            #         benchmark.extract_tables_queued(sample, top_n_rag_examples=True, n_examples=n_examples, collection=collection, out_of_sample=out_of_sample, result_dir="/pvc/benchmark_results/table_extraction/llm/final/real_tables/" + model_name.replace("/", "_") +"_vllm"+ f"__benchmark_{extractor_type}"+f"__temperature_{temperature}"+("_test" if test_run else "")+f"__top_{n_examples}_rag_examples"+("_out_of_sample" if out_of_sample else "")+f"__loop_{i}", no_think=no_think)
        else: 
            benchmark.extract_tables(sample, result_dir="/pvc/benchmark_results/table_extraction/llm/final/real_tables/openai/" + model_name.replace("/", "_") +"_azure"+ f"__benchmark_{extractor_type}"+f"__temperature_{temperature}"+("_test" if test_run else "")+f"__zero_shot__loop_{i}", no_think=no_think)
            benchmark.extract_tables(sample, static_example=True, result_dir="/pvc/benchmark_results/table_extraction/llm/final/real_tables/openai/" + model_name.replace("/", "_") +"_azure"+ f"__benchmark_{extractor_type}"+f"__temperature_{temperature}"+("_test" if test_run else "")+f"__static_example__loop_{i}", no_think=no_think)
            for n_examples in [
                1, 3, 5
                ]:
                print(f"Running benchmark with n_examples={n_examples}")
                benchmark.extract_tables(sample, random_examples=True, n_examples=n_examples, all_entries=entries, result_dir="/pvc/benchmark_results/table_extraction/llm/final/real_tables/openai/" + model_name.replace("/", "_") +"_azure"+ f"__benchmark_{extractor_type}"+f"__temperature_{temperature}"+("_test" if test_run else "")+f"__{n_examples}_random_examples__loop_{i}", no_think=no_think)
                for out_of_sample in [False, True]:
                    print(f"Running benchmark with out_of_sample={out_of_sample}")
                    benchmark.extract_tables(sample, top_n_rag_examples=True, n_examples=n_examples, collection=collection, out_of_sample=out_of_sample, result_dir="/pvc/benchmark_results/table_extraction/llm/final/real_tables/openai/" + model_name.replace("/", "_") +"_azure"+ f"__benchmark_{extractor_type}"+f"__temperature_{temperature}"+("_test" if test_run else "")+f"__top_{n_examples}_rag_examples"+("_out_of_sample" if out_of_sample else "")+f"__loop_{i}", no_think=no_think)
