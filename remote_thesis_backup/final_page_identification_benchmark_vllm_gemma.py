...
def main(model_name, classifier_type, test_run, n_loops, verbose, combine_system_prompts, no_think, tensor_parallel_size, batched, enforce_eager):
    ...
    for i in range(n_loops):
        ...
        if classifier_type == "four_classes":
            classifying_function(sample, result_dir="/pvc/benchmark_results/page_identification/final/llm/" + model_name.replace("/", "_") +"_vllm-0-9-1"+ f"__benchmark_{classifier_type}"+("_test" if test_run else "")+f"__zero_shot__loop_{i}", no_think=no_think)
            classifying_function(sample, law_context=True, result_dir="/pvc/benchmark_results/page_identification/final/llm/" + model_name.replace("/", "_") +"_vllm-0-9-1"+ f"__benchmark_{classifier_type}"+("_test" if test_run else "")+f"__law_context__loop_{i}", no_think=no_think)
            for n_examples in [
                1, 3, 
                # 5,
                # 7, 9, 11, 13
                ...
                classifying_function(sample, random_examples=True, n_examples=n_examples, all_entries=entries, result_dir="/pvc/benchmark_results/page_identification/final/llm/" + model_name.replace("/", "_") +"_vllm-0-9-1"+ f"__benchmark_{classifier_type}"+("_test" if test_run else "")+f"__{n_examples}_random_examples__loop_{i}", no_think=no_think)
                for out_of_sample in [True, False]:
                    print(f"Running classification with {n_examples} examples and out_of_sample={out_of_sample}")
                    classifying_function(sample, rag_examples=True, n_examples=n_examples, collection=collection, out_of_sample=out_of_sample, result_dir="/pvc/benchmark_results/page_identification/final/llm/" + model_name.replace("/", "_") +"_vllm-0-9-1"+ f"__benchmark_{classifier_type}"+("_test" if test_run else "")+f"__{n_examples}_rag_examples"+("_out_of_company" if out_of_sample else "")+f"__loop_{i}", no_think=no_think)
                    classifying_function(sample, top_n_rag_examples=True, n_examples=n_examples, collection=collection, out_of_sample=out_of_sample, result_dir="/pvc/benchmark_results/page_identification/final/llm/" + model_name.replace("/", "_") +"_vllm-0-9-1"+ f"__benchmark_{classifier_type}"+("_test" if test_run else "")+f"__top_{n_examples}_rag_examples"+("_out_of_company" if out_of_sample else "")+f"__loop_{i}", no_think=no_think)
        else:
            chosen_classes = [
                "Aktiva", 
                "Passiva", 
            ...
            for classification_type in chosen_classes:
                ...
                classifying_function(sample, classification_type=classification_type, result_dir="/pvc/benchmark_results/page_identification/final/llm/" + model_name.replace("/", "_") +"_vllm-0-9-1"+ f"__benchmark_{classifier_type}_{classification_type}"+("_test" if test_run else "")+f"__zero_shot__loop_{i}", no_think=no_think)
                classifying_function(sample, classification_type=classification_type, law_context=True, result_dir="/pvc/benchmark_results/page_identification/final/llm/" + model_name.replace("/", "_") +"_vllm-0-9-1"+ f"__benchmark_{classifier_type}_{classification_type}"+("_test" if test_run else "")+f"__law_context__loop_{i}", no_think=no_think)
                for n_examples in [
                    1, 
                    3, 
                    # 5, 
