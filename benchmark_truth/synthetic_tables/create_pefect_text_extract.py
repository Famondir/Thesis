...
def create_text_extract(df, output_path, column_names, n_columns=4, thin=False, span=True, unit_in_first_cell=False, unit='TEUR', add_enumeration=True, shuffle_rows=False, max_length=50, add_text_around=False, sum_in_same_row=False):
    df_with_sums = add_sum_rows(df, sum_in_same_row=sum_in_same_row)
    row_list = generate_row_list(df_with_sums, add_enumeration=add_enumeration)
    text_extract = "\n".join([" ".join(map(str, row)).strip().replace("nan ", "") for row in row_list]) # hier sind noch die ganzen Angaben drin, wie groß eine Gruppe ist. Es braucht eine spezielfunktion wie die für html_table
    return text_extract    
...
if __name__ == "__main__":
    ...
    csv_dir = "/home/simon/Documents/data_science/Thesis/benchmark_truth/real_tables_extended"

    csv_files = glob.glob(os.path.join(csv_dir, "*.csv"))
    csv_data = []
    for file in csv_files:
        df = pd.read_csv(file)
        csv_data.append({"filename": os.path.basename(file), "dataframe": df})

    test_run = True
    if test_run:
        create_text_extract(csv_data[0]['dataframe'], './benchmark_truth/real_tables/aktiva_table/'+csv_data[0]['filename'][:-4]+'.txt', column_names, n_columns=5, thin=False, span=False, unit_in_first_cell=False, unit='Mio. EUR', max_length=50, add_text_around=True, sum_in_same_row=False)
